...
leftpad " " 1 = " "
leftpad str x
 | length str < x  = take (x - length str) (cycle " ") ++ str
 | otherwise       = str
...
countdown n = "Ready! " ++ (number n) ++ " Liftoff!"

number :: Integer -> String
number 1 = show (1) ++ "... "
number n = show (n) ++ "... "  ++ (number (n-1))
...
isPrimehelper n k =  if (mod n k == 0)
                     then
                       if (n == k)
                       then True
                       else False
