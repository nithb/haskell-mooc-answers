...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if n <= length xs - 1 then drop (length xs - n) xs else xs
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = if i /= 0 then (take i xs) ++ [x] ++ drop (i + 1) xs else [x] ++ tail xs
...
safeIndex xs i = if length xs >= (i + 1) && i >= 0 then Just (xs !! i)  else Nothing
